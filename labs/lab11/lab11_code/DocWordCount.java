    ...
    public static class DocWordCountMap extends Mapper<Text, Text, Text, LongWritable> {
        ...
        public void map(Text key, Text value, Context context)
            ...
            while (matcher.find()) { //
				...
				if(!contained.contains(x)) { // * if string isn't already in hashset,
		    		context.write(word, ONE); // add it with value 1, since it's being counted once
		    		contained.add(x); // 
				} else {
                    context.write(word, new LongWritable(0L)); // otherwise give value 0 to not count
				}
            }
